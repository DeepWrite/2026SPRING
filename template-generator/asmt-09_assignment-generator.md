---
title: 과제-09 기말과제 최종본 템플릿 생성기
layout: home
nav_order: 1
parent: 과제-09 기말과제 최종본
permalink: /assignment-generator/asmt-09
---

<div class="assignment-gen">
  <h2>과제-09 기말과제 최종본 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-asmt-09" class="tab active" role="tab" aria-selected="true">과제-09</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-asmt-09">
    ...
    <button id="btn-asmt-09" class="btn">과제-09 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>asmt-09-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-asmt-09");
    ...
    btn.addEventListener("click", async () => {
      ...
      const filename = `asmt-09-${classNo}-${serialNo}-${name}.md`;
      try {
        const content = await loadAssignmentTemplate("asmt-09-template", name, classNo, serialNo);
