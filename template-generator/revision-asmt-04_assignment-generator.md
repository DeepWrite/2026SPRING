...
layout: home
nav_order: 1
parent: (개선) 과제-04 5-6단락 논증에세이 개선
permalink: /assignment-generator/revision-asmt-04
---

<div class="assignment-gen">
  <h2>(개선) 과제-04 5-6단락 논증에세이 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-revision-asmt-04" class="tab active" role="tab" aria-selected="true">개선 과제-04</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-revision-asmt-04">
    ...
    <button id="btn-revision-asmt-04" class="btn">(개선) 과제-04 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>revision-asmt-04-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function revisionAsmt04Template(name, classNo, serialNo) {
    return [
      "---",
      `title: (개선) 과제-04 5-6단락 논증에세이 ${classNo}-${serialNo} ${name}`,
      "layout: home",
      "nav_order: 99",
      `parent: ${classNo}-${serialNo} ${name} (과제-04)`,
      `permalink: /asmt-04/${classNo}-${serialNo}/revision`,
      "---",
      "",
      `# (개선) 과제-04 5-6단락 논증에세이 ${classNo}-${serialNo} ${name} `,
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-revision-asmt-04");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `revision-asmt-04-${classNo}-${serialNo}-${name}.md`;
      const content = revisionAsmt04Template(name, classNo, serialNo);
