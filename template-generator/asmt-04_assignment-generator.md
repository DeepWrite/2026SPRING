---
title: 과제-04 5-6단락 논증에세이 템플릿 생성기
layout: home
nav_order: 1
parent: 과제-04 5-6단락 논증에세이
permalink: /assignment-generator/asmt-04
---

<div class="assignment-gen">
  <h2>과제-04 5-6단락 논증에세이 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-asmt-04" class="tab active" role="tab" aria-selected="true">과제-04</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-asmt-04">
    ...
    <button id="btn-asmt-04" class="btn">과제-04 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>asmt-04-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function asmt04Template(name, classNo, serialNo) {
    const navOrder = String(parseInt(serialNo, 10) || serialNo).trim();

    return [
      "---",
      `title: ${classNo}-${serialNo} ${name} (과제-04)`,
      "layout: home",
      `nav_order: ${navOrder}`,
      "parent: 과제-04 5-6단락 논증에세이",
      `permalink: /asmt-04/${classNo}-${serialNo}`,
      "---",
      "",
      `# 과제-04 5-6단락 논증에세이 ${classNo}-${serialNo} ${name} `,
      "",
      "---",
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-asmt-04");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `asmt-04-${classNo}-${serialNo}-${name}.md`;
      const content = asmt04Template(name, classNo, serialNo);
