---
title: 과제-02 쟁점과 딜레마 분석 템플릿 생성기
layout: home
nav_order: 1
parent: 과제-02 쟁점과 딜레마 분석
permalink: /assignment-generator/asmt-02
---

<div class="assignment-gen">
  <h2>과제-02 쟁점과 딜레마 분석 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-asmt-02" class="tab active" role="tab" aria-selected="true">과제-02</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-asmt-02">
    ...
    <button id="btn-asmt-02" class="btn">과제-02 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>asmt-02-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function asmt02Template(name, classNo, serialNo) {
    const navOrder = String(parseInt(serialNo, 10) || serialNo).trim();

    return [
      "---",
      `title: ${classNo}-${serialNo} ${name} (과제-02)`,
      "layout: home",
      `nav_order: ${navOrder}`,
      "parent: 과제-02 쟁점과 딜레마 분석",
      `permalink: /asmt-02/${classNo}-${serialNo}`,
      "---",
      "",
      `# 과제-02 쟁점과 딜레마 분석 ${classNo}-${serialNo} ${name} `,
      "",
      "## 1. 관심 주제 및 일반적 배경",
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-asmt-02");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `asmt-02-${classNo}-${serialNo}-${name}.md`;
      const content = asmt02Template(name, classNo, serialNo);
