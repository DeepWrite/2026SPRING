...
<div class="assignment-gen">
  <h2>(개선) 과제-07 개인별 논증 구조 작성하기 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-revision-asmt-07" class="tab active" role="tab" aria-selected="true">개선 과제-07</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-revision-asmt-07">
    ...
    <button id="btn-revision-asmt-07" class="btn">(개선) 과제-07 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>revision-asmt-07-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function revisionAsmt07Template(name, classNo, serialNo) {
    return [
      "---",
      `title: (개선) 과제-07 개인별 논증 구조 작성하기 ${classNo}-${serialNo} ${name}`,
      "layout: home",
      "nav_order: 99",
      `parent: ${classNo}-${serialNo} ${name} (과제-07)`,
      `permalink: /asmt-07/${classNo}-${serialNo}/revision`,
      "---",
      "",
      `# (개선) 과제-07 개인별 논증 구조 작성하기 ${classNo}-${serialNo} ${name} `,
      "",
      "## 개선 사항 메모",
      "",
      "이러저러한 점을 반영하고 개선하였다.",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-revision-asmt-07");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `revision-asmt-07-${classNo}-${serialNo}-${name}.md`;
      const content = revisionAsmt07Template(name, classNo, serialNo);
