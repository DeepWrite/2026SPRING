---
title: 과제-07 개인별 논증 구조 작성하기 템플릿 생성기
layout: home
nav_order: 1
parent: 과제-07 개인별 논증 구조 작성하기
permalink: /assignment-generator/asmt-07
---

<div class="assignment-gen">
  <h2>과제-07 개인별 논증 구조 작성하기 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="과제 선택">
    <button id="tab-asmt-07" class="tab active" role="tab" aria-selected="true">과제-07</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-asmt-07">
    ...
    <button id="btn-asmt-07" class="btn">과제-07 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>asmt-07-수강반-연번-이름.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function asmt07Template(name, classNo, serialNo) {
    const navOrder = String(parseInt(serialNo, 10) || serialNo).trim();

    return [
      "---",
      `title: ${classNo}-${serialNo} ${name} (과제-07)`,
      "layout: home",
      `nav_order: ${navOrder}`,
      "parent: 과제-07 개인별 논증 구조 작성하기",
      `permalink: /asmt-07/${classNo}-${serialNo}`,
      "---",
      "",
      `# 과제-07 개인별 논증 구조 작성하기 ${classNo}-${serialNo} ${name}`,
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-asmt-07");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `asmt-07-${classNo}-${serialNo}-${name}.md`;
      const content = asmt07Template(name, classNo, serialNo);
