---
title: 코멘트-07 템플릿 생성기
layout: home
nav_order: 1
parent: 과제-07에 대한 코멘트
permalink: /assignment-generator/comment-07
---

<div class="assignment-gen comment-gen">
  <h2>과제-07 코멘트 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="코멘트 선택">
    <button id="tab-comment-07" class="tab active" role="tab" aria-selected="true">코멘트-07</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-comment-07">
    ...
    <button id="btn-comment-07" class="btn">코멘트-07 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>comment-07-수강반-작성자연번-작성자이름-대상연번.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function comment07Template(classNo, writerSerialNo, writerName, targetSerialNo, targetName) {
    const navOrder = String(parseInt(writerSerialNo, 10) || writerSerialNo).trim();

    return [
      "---",
      `title: ${classNo}-${writerSerialNo} ${writerName}의 코멘트 ${targetSerialNo} (과제-07)`,
      "layout: home",
      `nav_order: ${navOrder}`,
      `parent: ${classNo}-${targetSerialNo} ${targetName} (과제-07)`,
      `permalink: /asmt-07/${classNo}-${targetSerialNo}/comment-${classNo}-${writerSerialNo}`,
      "---",
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-comment-07");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `comment-07-${classNo}-${writerSerialNo}-${writerName}-${targetSerialNo}.md`;
      const content = comment07Template(classNo, writerSerialNo, writerName, targetSerialNo, targetName);
