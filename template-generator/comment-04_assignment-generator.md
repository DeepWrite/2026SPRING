...
layout: home
nav_order: 1
parent: 과제-04에 대한 코멘트
permalink: /assignment-generator/comment-04
---

<div class="assignment-gen comment-gen">
  <h2>과제-04 코멘트 템플릿 생성기</h2>

  <div class="tabs" role="tablist" aria-label="코멘트 선택">
    <button id="tab-comment-04" class="tab active" role="tab" aria-selected="true">코멘트-04</button>
  </div>

  <div class="panel" role="tabpanel" aria-labelledby="tab-comment-04">
    ...
    <button id="btn-comment-04" class="btn">코멘트-04 템플릿 다운로드</button>
    <p class="text-small text-grey-dk-100">
      파일명: <code>comment-04-수강반-작성자연번-작성자이름-대상연번.md</code>
    </p>
  </div>
</div>
...
<script>
  ...
  function comment04Template(classNo, writerSerialNo, writerName, targetSerialNo, targetName) {
    const navOrder = String(parseInt(writerSerialNo, 10) || writerSerialNo).trim();

    return [
      "---",
      `title: ${classNo}-${writerSerialNo} ${writerName}의 코멘트 ${targetSerialNo} (과제-04)`,
      "layout: home",
      `nav_order: ${navOrder}`,
      `parent: ${classNo}-${targetSerialNo} ${targetName} (과제-04)`,
      `permalink: /asmt-04/${classNo}-${targetSerialNo}/comment-${classNo}-${writerSerialNo}`,
      "---",
      "",
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const btn = document.getElementById("btn-comment-04");
    ...
    btn.addEventListener("click", () => {
      ...
      const filename = `comment-04-${classNo}-${writerSerialNo}-${writerName}-${targetSerialNo}.md`;
      const content = comment04Template(classNo, writerSerialNo, writerName, targetSerialNo, targetName);
